 ...
 *
 * Implementa la forma esperada del CollabRepository antes de que
 * sea implementado en src/repositories/index.ts.
 */

// ─────────────────────────────────────────────────────────────────
// Tipos e interfaces esperadas del repositorio
// ─────────────────────────────────────────────────────────────────
...
/**
 * Interfaz del repositorio de colaboración.
 * Define el contrato que debe cumplir la implementación real con Drizzle.
 */
interface ICollabRepository {
  save_update(document_id: string, update: Uint8Array): Promise<CollabUpdate>;
  get_updates(document_id: string): Promise<CollabUpdate[]>;
  save_snapshot(document_id: string, snapshot: Uint8Array, state_vector: Uint8Array): Promise<CollabSnapshot>;
...
function make_mock_repository(): ICollabRepository {
  return {
    save_update: vi.fn(),
    get_updates: vi.fn(),
    save_snapshot: vi.fn(),
...
// ─────────────────────────────────────────────────────────────────
// Implementación del servicio de persistencia para testear
// ─────────────────────────────────────────────────────────────────

/**
 * Servicio de persistencia de Y.js.
 * Aplica updates acumulados sobre el snapshot base para reconstruir el doc.
 */
class CollabPersistenceService {
  constructor(private readonly repository: ICollabRepository) {}

  /**
   * Persiste un update Y.js incremental en la DB.
   */
  ...
  /**
   * Recarga el estado Y.js completo desde DB:
   * toma el último snapshot y aplica todos los updates posteriores.
   */
  async load_document(document_id: string): Promise<Y.Doc> {
    const doc = new Y.Doc();

    // 1. Aplicar snapshot base si existe
    const snapshot = await this.repository.get_latest_snapshot(document_id);
    if (snapshot) {
      Y.applyUpdate(doc, snapshot.snapshot_data);
    }

    // 2. Aplicar updates incrementales
  ...
  /**
   * Toma un snapshot del estado actual y opcionalmente elimina updates previos.
   */
...
describe("CollabPersistenceService", () => {
  let mock_repo: ICollabRepository;
  ...
  describe("persist_update", () => {
    ...
    it("should_call_save_update_with_document_id_and_data", async () => {
      ...
      vi.mocked(mock_repo.save_update).mockResolvedValue(expected_result);
    ...
    it("should_return_saved_update_from_repository", async () => {
      ...
      vi.mocked(mock_repo.save_update).mockResolvedValue(saved);
    ...
    it("should_propagate_repository_errors", async () => {
      ...
      vi.mocked(mock_repo.save_update).mockRejectedValue(new Error("DB write failed"));

      // Act & Assert
      await expect(service.persist_update(document_id, update)).rejects.toThrow(
        "DB write failed"
      );
    });
  });
  ...
  describe("load_document", () => {
    ...
    it("should_return_empty_ydoc_when_no_snapshot_or_updates", async () => {
      ...
      vi.mocked(mock_repo.get_latest_snapshot).mockResolvedValue(null);
      vi.mocked(mock_repo.get_updates).mockResolvedValue([]);
    ...
    it("should_apply_snapshot_when_available", async () => {
      ...
      vi.mocked(mock_repo.get_latest_snapshot).mockResolvedValue({
        id: "snap-1",
        document_id,
        snapshot_data,
  ...
  describe("take_snapshot", () => {
    ...
    it("should_call_save_snapshot_with_encoded_state", async () => {
      ...
      vi.mocked(mock_repo.save_snapshot).mockResolvedValue(expected_snap);
      ...
      const [called_id, called_snap, called_sv] = vi.mocked(mock_repo.save_snapshot).mock.calls[0];
    ...
    it("should_produce_snapshot_data_that_restores_document", async () => {
      ...
      vi.mocked(mock_repo.save_snapshot).mockImplementation(
        async (_id, snap_data, sv) => {
    ...
    it("should_propagate_errors_from_save_snapshot", async () => {
      // Arrange
      const doc = new Y.Doc();
      vi.mocked(mock_repo.save_snapshot).mockRejectedValue(new Error("Snapshot write failed"));

      // Act & Assert
      await expect(service.take_snapshot("doc-fail", doc)).rejects.toThrow(
